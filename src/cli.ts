...
program
  .command("summary")
  .description("Get a day's sleep score, sleep periods, readiness, and activity")
  .option("--date <date>", "Date in YYYY-MM-DD format", today())
  .action(async (opts) => {
    const client = OuraClient.fromEnv();
...
program
  .command("workouts")
  .description("Get workouts (manual, auto-detected, confirmed, and heart-rate-derived)")
  .option("--date <date>", "Date in YYYY-MM-DD format", today())
  .action(async (opts) => {
    const client = OuraClient.fromEnv();
...
program
  .command("sessions")
  .description("Get guided sessions (breathing, meditation, nap, relaxation, rest)")
  .option("--date <date>", "Date in YYYY-MM-DD format", today())
  .action(async (opts) => {
    const client = OuraClient.fromEnv();
